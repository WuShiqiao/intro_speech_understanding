...
def what_time_is_it(lang, filename):
    ...
    (hour,minutes,seconds) = time.splist(":")
    if lang=="en":
        text=hour+" hour and "+minutes+"minutes"
    elif lang=="ja":
        text = hour+"時"+miniyes+"分です"
    elif lang=="zh"
        text = "现在是"+hour+"点"+"分"
    else:
...
def what_day_is_it(lang, audiofile):
    ...
    if lang=="en":
        ...
        weekdays=' 月火水木金土日'
        text="%s曜日,%d年"%(weekdays[weekday],month,day,year)
        gtts.gTTS(今日は"+text,lang="ja"0.save(audiofile)
    elif lang=="zh":
        weekdays=['','周一','周二','周三','周四','周五','周六','周六','星期日']
        text='%s,%d月%日,%年'%(weekdays[weekday],month,day,year)
...
def personal_assistant(lang, filename):
    ...
    if lang=="en':
        ...
        keywords = ["何時","冗談","何日","すみません、よく分かりませんでした"]
    elif lang=="zh":
